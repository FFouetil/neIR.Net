...
{
    /// <summary>Summary for the PictureBox Ctrl</summary>
    public class ZoomablePictureBox : System.Windows.Forms.UserControl
    {
        #region Members

        private PixelBox PicBox;
        ...
        private double ZOOMFACTOR = 2;	// = 2x smaller or larger
        private int MaxZoomOut = 1 << 2; // stops zooming out at specified power of 2		
        private int MaxZoomIn = 1 << 6; // stops zooming out at in by specified power of 2				
        ...
        [Browsable(true)]
        public System.Drawing.Image Image { get { return PicBox.Image; }
            set { PicBox.Image = value;
            if (value != null) { m_aspectRatio = (float)value.Width / (float)value.Height; ApplyImageRatio(); }
            }
        }
        ...
        private bool PicBoxIsBiggerThanImage()
        { 
            return (PicBox.Size.Width > PicBox.Image.Size.Width);
        }

        private void UpdateInterpolationMode()
        {

            if (PicBoxIsBiggerThanImage())
            {
                PicBox.InterpolationMode = InterpolationMode.NearestNeighbor;
            }
            else
            {
                PicBox.InterpolationMode = InterpolationMode.Default;                    
            }
               
        }

        private void ApplyImageRatio()
        {              
            if (m_aspectRatio > 1)
            {
                PicBox.Height = (int)(Height);
                PicBox.Width = (int)(PicBox.Height * m_aspectRatio);
                    
            }
            else if (m_aspectRatio < 1){
                PicBox.Width = (int)(Width);
                PicBox.Height = (int)(PicBox.Width * m_aspectRatio);
            }
        }
        ...
        private void ZoomIn()
        {
            if ((PicBox.Width < (MaxZoomIn * OuterPanel.Width)) &&
                (PicBox.Height < (MaxZoomIn * OuterPanel.Height)))
            {
        ...
        {
            if ((PicBox.Width > (OuterPanel.Width / MaxZoomOut)) &&
                (PicBox.Height > (OuterPanel.Height / MaxZoomOut)))
            {
                UpdateInterpolationMode();
                PicBox.SizeMode = PictureBoxSizeMode.Zoom;
                PicBox.Width = Convert.ToInt32(PicBox.Width / ZOOMFACTOR);
        ...
        private void PicBox_MouseWheel(object sender, MouseEventArgs e)
        {
            if (e.Delta < 0)
            {
                ...
                ZoomIn();
                //throw new Exception("Crash here please!");
            }
            else
            {
